...
public class Gui implements InventoryProvider {
                                ...
                                e -> {
                                    if (e.getCurrentItem() != null)
                                        buy(player, finalSlot, e.getAction() == InventoryAction.MOVE_TO_OTHER_INVENTORY
                                                ...
                                                .mapToInt(ItemStack::getAmount).sum() : e.getCurrentItem().getMaxStackSize());
                                }));
            } else {
    ...
    private void buy(Player player, int slot, int amount) {
        Main.executor.execute(() -> {
            ...
            if (toWithdraw > 0) {
                if (toWithdraw >= product.getItem().getMaxStackSize()) {
                    ...
                    player.sendMessage(Config.inst.getMessage("Messages.less-stack"));
                }
            } else {
